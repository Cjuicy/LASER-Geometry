...
This workspace includes an experimental geometry-aware segmentation path for LASER. The first stage only changes the source of segment labels (`depth` vs `geometry`) and keeps the original LASER scale estimation, scale propagation, and loop closure logic unchanged.
...
See `docs/GEOMETRY_M0.md` for the implementation scope and `docs/CLOUD_RUN.md` for cloud smoke-test commands.
