...
from .alignment_pipeline_trace import ScaleTraceCollector
...
class StreamingWindowEngine(VanillaEngine):
    def __init__(
        ...
        if top_conf_percentile is None or not 0.0 < top_conf_percentile <= 1.0:
            raise ValueError("top_conf_percentile must be in (0, 1].")
        if debug_sample_interval <= 0:
            raise ValueError("debug_sample_interval must be positive.")
        ...
        self.top_conf_percentile = 1 - top_conf_percentile      # 反转逻辑，就是提取置信度最高的top_conf_percentile比例的点
    ...
    def _build_depth_segment_graph(self, local_points, conf, segmentation_trace=None):
        ...
        return build_depth_sp_graph(local_points_np[..., -1], **kwargs)

    # 🌟辅助：新增 geometry segmentation 路径，只有这里会传入 point_map/intrinsic/normal_method。
    def _build_geometry_segment_graph(
        self,
        local_points,
        conf,
        ref_intrinsic,
        ...
        if segmentation_trace is not None:
            kwargs["segmentation_trace"] = segmentation_trace
        return build_geometry_sp_graph(local_points_np[..., -1], **kwargs)

    def _record_alignment_pipeline_window(
        self,
        *,
        window_index,
        working_window,
        segmentation_trace,
        scale_trace,
        mutual_conf_mask=None,
    ):
        if not self.debug_alignment or segmentation_trace is None:
            return None

        frame_count, height, width = segmentation_trace["merged_labels"].shape
        window_start = window_index * (self.window_size - self.overlap)
        if mutual_conf_mask is None:
            mutual_conf_mask = np.empty((0, height, width), dtype=bool)
        ...
        payload = {
            **segmentation_trace,
            ...
            "mutual_confidence_masks": mutual_conf_mask,
            **scale_trace.to_arrays(),
        }
        return self.alignment_debug_recorder.record_pipeline_window(
            ...
            metadata={
                ...
                "sample_interval": self.debug_sample_interval,
                "confidence_retained_fraction": self.confidence_retained_fraction,
    ...
    def _registration_worker(self):
        ...
        while True:
            ...
            pipeline_debug = self.debug_alignment and self.depth_refine
            segmentation_trace = {} if pipeline_debug else None
            scale_trace = ScaleTraceCollector() if pipeline_debug else None
            mutual_conf_mask = None
            segmentation_trace_kwargs = (
                {"segmentation_trace": segmentation_trace}
                if segmentation_trace is not None
                else {}
            )
            scale_trace_kwargs = {"trace": scale_trace} if scale_trace is not None else {}
            ...
            # 5️⃣第一个窗口分支（没有前窗，所以不做相邻的窗口配准）
                ...
                if self.depth_refine:
                    ...
                    if scale_trace is not None:
                        scale_trace.capture_segment_states(tgt_sp_graph)

            self._record_alignment_pipeline_window(
